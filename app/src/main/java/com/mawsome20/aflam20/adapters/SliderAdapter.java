...
import com.mawsome20.aflam20.WebViewActivity;
...
import static com.mawsome20.aflam20.utils.MyAppClass.getContext;

public class SliderAdapter extends CardSliderAdapter<mix_movies_series> {
    public SliderAdapter(@NotNull ArrayList<mix_movies_series> items) {
        super(items);
    }
    ...
    public void bindView(int i, @NotNull View view, @Nullable final mix_movies_series slide) {
        if (slide != null){
            ...
            Picasso.get().load(slide.getPic_profile()).fit().centerCrop().into(imageView);
