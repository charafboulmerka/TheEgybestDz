...
import com.mawsome20.aflam.adapters.need_later.CastCrewAdapter;
import com.mawsome20.aflam.adapters.need_later.CommentsAdapter;
import com.mawsome20.aflam.adapters.EpisodeAdapter;
import com.mawsome20.aflam.adapters.DownloadAdapter;
import com.mawsome20.aflam.adapters.ServerApater;
import com.mawsome20.aflam.models.CastCrew;
import com.mawsome20.aflam.models.GetCommentsModel;
import com.mawsome20.aflam.models.CommonModels;
import com.mawsome20.aflam.models.Program;
import com.mawsome20.aflam.models.SubtitleModel;
...
import com.mawsome20.aflam.new_conception.models.Seasons;
...
import com.mawsome20.aflam.new_conception.models.strm_link;
 ...
 */
    ...
    private void addToFavFC(){
        ...
        mKey.setValue(new mix_movies_series(id,finalTitle,finalPicCover,finalPicProfile,finalGenres,finalYear,finalCategory,type))
    ...
    private void getSerieFC(){
        strGenre = "";
        mRef.child("Series").orderByKey().equalTo(id).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                finalPicCover = serie.getPic_cover();
                ...
                Picasso.get().load(serie.getPic_cover()).fit().centerCrop().placeholder(R.drawable.album_art_placeholder_large)
                        .into(posterIv);
                Picasso.get().load(serie.getPic_profile()).fit().centerCrop().placeholder(R.drawable.poster_placeholder)
                        .into(thumbIv);
                if (!serie.getGeners().contains("|")){
                    strGenre = Tools.getArName(serie.getGeners());
                }else {
                    List<String> gr = Arrays.asList(serie.getGeners().split("\\|"));
                    if (gr.size()!=0){
                            for (String i:gr){
                                if (strGenre.equals("")){
                                    strGenre+=Tools.getArName(i);
                                }else {
                                    strGenre=strGenre+"، "+Tools.getArName(i);
                                }
                            }
    ...
    private void getMovieFC(){
        strGenre = "";
        mRef.child("Movies").orderByKey().equalTo(id).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                shimmerFrameLayout.stopShimmer();
                shimmerFrameLayout.setVisibility(GONE);
                ...
                finalPicCover = movie.getPic_cover();
                ...
                if (!movie.getGeners().contains("|")){
                    strGenre = Tools.getArName(movie.getGeners());
                }else {
    ...
    private class getMoviesInBackground extends AsyncTask<String, Integer, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            mRef.child("Movies").limitToLast(20).addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    try{
    ...
    private class getSeriesInBackground extends AsyncTask<String, Integer, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            // get the string from params, which is an array
            String myGenres = params[0];
            mRef.child("Series").limitToLast(20).addListenerForSingleValueEvent(new ValueEventListener() {
